...
public class HqInventoryDslRepository {
	...
	public List<HqIngredientDto> findHqInventoryByFilters(Integer categoryId, String keyword, LocalDate startDate,
			LocalDate endDate, PageRequest pageRequest, String sortOption) {
		...
		if (startDate != null)
			builder.and(q.receivedDate.goe(startDate));
		if (endDate != null)
			builder.and(q.receivedDate.loe(endDate));
		...
		return queryFactory
				.select(Projections.bean(HqIngredientDto.class, q.id, q.quantity, q.minimumOrderUnit, q.unitCost,
						q.expiredDate, q.receivedDate, q.ingredient.name.as("ingredientName"),
						q.ingredient.unit.as("unit"), q.category.name.as("categoryName"), store.name.as("storeName"),
						s.minQuantity.as("minquantity"), q.category.id.as("categoryId"),
						q.ingredient.id.as("ingredientId")))
				.from(q).leftJoin(q.ingredient).leftJoin(q.category).leftJoin(q.store, store).leftJoin(s)
				.on(s.store.id.eq(store.id).and(s.ingredient.eq(q.ingredient))).where(builder)
	...
	public List<HqIngredientDto> findHqInventoryExpirationByFilters(Integer categoryId, String keyword,
	...
	// 유통기한 목록 개수 조회
	public long countHqInventoryExpirationByFilters(Integer categoryId, String keyword, LocalDate startDate,
			LocalDate endDate) {
		QHqIngredient q = QHqIngredient.hqIngredient;
		BooleanBuilder builder = new BooleanBuilder();
	...
	public void updateHqIngredient(HqIngredientDto dto) {
		...
		JPAUpdateClause clause = queryFactory.update(q).set(q.category, category).set(q.ingredient, ingredient)
				...
				.set(q.receivedDate, dto.getReceivedDate()).set(q.reservedQuantity, dto.getReservedQuantity())
	...
	// 폐기 개수 조회
	public int countHqDisposals(Integer storeId, Integer categoryId, String status, LocalDate startDate,
			LocalDate endDate, String keyword) {
		QDisposal d = QDisposal.disposal;
		BooleanBuilder builder = new BooleanBuilder();

		if (storeId != null)
			builder.and(d.store.id.eq(storeId));
		if (categoryId != null)
	...
	// 폐기목록 조회
	public List<Disposal> selectHqDisposalListByFiltersPaging(Integer storeId, Integer categoryId, String status,
			LocalDate startDate, LocalDate endDate, int offset, int limit, String sortOption, String keyword) {
		...
		return queryFactory.selectFrom(d).where(builder).orderBy(orders.toArray(new OrderSpecifier[0])).offset(offset)
				.limit(limit).fetch();
	}
	...
	public void updateDisposalStatus(List<Integer> disposalIds, String status, String memo) {
		...
		queryFactory.update(disposal).set(disposal.status, status).set(disposal.memo, memo)
				.where(disposal.id.in(disposalIds)).execute();
	}
	...
	public List<Ingredient> getAllIngredients() {
		...
		return queryFactory.selectFrom(ingredient).leftJoin(ingredient.category, category).fetch();
	}
	...
	public List<InventoryRecordDto> findHqInventoryRecords(String changeType, PageRequest pageRequest) {
		...
		return queryFactory
				.select(Projections.bean(InventoryRecordDto.class, ir.id, ir.quantity, ir.changeType, ir.memo, ir.date,
						ingredient.id.as("ingredientId"), ingredient.name.as("ingredientName"),
						category.name.as("categoryName"), store.id.as("storeId"), store.name.as("storeName")))
				.from(ir).leftJoin(ir.ingredient, ingredient).leftJoin(ingredient.category, category)
				.leftJoin(ir.store, store).where(builder).orderBy(ir.date.desc()).offset(pageRequest.getOffset())
	...
	public long countHqSettingsByFilters(Integer storeId, Integer categoryId, String keyword) {
		...
		if (categoryId != null)
			builder.and(setting.ingredient.category.id.eq(categoryId));
	...
	public List<StoreIngredientSettingDto> findHqSettingsByFilters(Integer storeId, Integer categoryId, String keyword,
		...
		return queryFactory
				.select(Projections.bean(StoreIngredientSettingDto.class, setting.id, setting.minQuantity,
						setting.maxQuantity, setting.ingredient.id.as("ingredientId"),
						setting.ingredient.category.name.as("categoryName")))
				.from(setting).where(builder).offset(offset).limit(limit).fetch();
	}
	...
	public InventoryExpireSummaryDto findExpireSummaryTop3WithCountMerged(String startDate, String endDate) {
		...
		List<InventoryExpireSummaryDto.Item> top3 = queryFactory
				.select(Projections.bean(InventoryExpireSummaryDto.Item.class, hq.ingredient.name.as("ingredientName"),
						hq.category.name.as("categoryName"), hq.quantity.as("remainQuantity"),
						hq.expiredDate.stringValue().as("expiredDate")))
				.from(hq)
				.where(hq.quantity.gt(0), sDate != null ? hq.expiredDate.goe(sDate) : null,
						eDate != null ? hq.expiredDate.loe(eDate) : null)
				.orderBy(hq.expiredDate.asc(), hq.quantity.desc()).limit(3).fetch();

		// 전체 임박재고 건수
		Long totalCount = queryFactory.select(hq.count()).from(hq).where(hq.quantity.gt(0),
				sDate != null ? hq.expiredDate.goe(sDate) : null, eDate != null ? hq.expiredDate.loe(eDate) : null)
				.fetchOne();

		// D-1 (내일 유통기한)
		Long d1Count = queryFactory.select(hq.count()).from(hq)
				.where(hq.quantity.gt(0), hq.expiredDate.eq(today.plusDays(1))).fetchOne();

		// D-day (오늘 유통기한)
		Long todayCount = queryFactory.select(hq.count()).from(hq).where(hq.quantity.gt(0), hq.expiredDate.eq(today))
				.fetchOne();
	...
	public DisposalSummaryDto findDisposalSummaryTop3WithCountMerged(String startDate, String endDate) {
		...
		List<DisposalSummaryDto.Item> top3 = queryFactory
				.select(Projections.bean(DisposalSummaryDto.Item.class, d.ingredient.name.as("ingredientName"),
						d.ingredient.category.name.as("categoryName"), d.quantity.sum().intValue().as("quantity"), // 전체
																													// 수량
																													// 합산
						d.requestedAt.max().stringValue().as("requestedAt") // 가장 최근 요청일자
				)).from(d).where(sDate != null ? d.requestedAt.goe(sDate) : null,
						eDate != null ? d.requestedAt.loe(eDate) : null, d.status.eq("대기") // 필수 추가!
				).groupBy(d.ingredient.name, d.ingredient.category.name).orderBy(d.quantity.sum().desc()).limit(3)
				.fetch();

		Long totalCount = queryFactory.select(d.count()).from(d).where(sDate != null ? d.requestedAt.goe(sDate) : null,
				eDate != null ? d.requestedAt.loe(eDate) : null, d.status.eq("대기") // 필수 추가!
		).fetchOne();
